...
function ElementBox(selector) {
  ...
  if (selector.startsWith('<')) {
    this.el[0] = document.createElement(selector.substring(1, selector.length - 1)); // 꺽쇠 안 문자열만 추출
  } else {
...
ElementBox.prototype.append = function(childBox) {
  ...
  // 자식 태그는 제거한다.
  for (let child of childBox.el) { // createElement로 생성한 태그는 부모가 없으므로 제외
...
ElementBox.prototype.appendTo = function(parentBox) {
  // 자식 태그를 복제해서 각 부모 태그에 붙인다.
  for (let parentTag of parentBox.el) { // parents가 배열이 아니므로 그냥 parents쓰면 오류!
    // 자식들이 들어있는 상자에서 자식을 한 개씩 꺼내 복제하여 각 부모의 자식으로 붙인다.
  ...
  // 자식 태그는 제거한다.
  for (let child of this.el) { // createElement로 생성한 태그는 부모가 없으므로 제외
    if (child.parentElement != null || child.parentElement != undefined) {
      child.parentElement.removeChild(child);
    }
  }

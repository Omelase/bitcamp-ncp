...
app.get('/hello', (req, res) => {
  res.set('Access-Control-Allow-Origin', '*'); // 응답 헤더 설정
  res.set('Content-Type', 'text/plain; charset=UTF-8'); // 응답 헤더 설정

  setTimeout(()=> {
    res.send("Hello!");
  }, 3000); // 3초 후 응답. setTimeout이 실행되기 전에 app.get실행이 끝남
}); 
...
app.get('/boards', (req, res) => {
  ...
  setTimeout(() => {
    res.send(JSON.stringify(arr)); // 객체에서 json으로 바꿔서 응답
  }, 3000);
}); 
